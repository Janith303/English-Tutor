...
import { idiomsQuestions } from "../../data/idiomsQuestions";

export default function IdiomsQuizGame() {
  ...
  const handleNext = () => {
    if (currentQuestion < idiomsQuestions.length - 1) {
  ...
  const handleQuizComplete = () => {
    navigate("/idioms-quiz/result", {
      state: { answers, questions: idiomsQuestions },
    });
  };

  const progress = ((currentQuestion + 1) / idiomsQuestions.length) * 100;
  const question = idiomsQuestions[currentQuestion];
    ...
    <div className="min-h-screen bg-[#F5F9FF]">
      ...
      <main className="max-w-4xl mx-auto px-6 py-6">
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-sm mb-5">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[#0F172A] font-semibold">
              Question {currentQuestion + 1} of {idiomsQuestions.length}
            </span>
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          >
            {currentQuestion === idiomsQuestions.length - 1 ? "Finish Quiz" : "Next"}
