        ...
        <div className="w-full max-w-4xl bg-white/90 backdrop-blur-md rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-white p-8 md:p-12 animate-in fade-in zoom-in duration-500">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 mt-2 font-medium">
              This helps us tailor your English learning experience
            </p>
          </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {interestAreas.map((area) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`p-3 rounded-xl mr-4 ${selectedInterest?.id === area.id ? "bg-blue-600 text-white" : "bg-slate-50 text-slate-400"}`}
                      >
                        <Icon className="w-5 h-5" />
                      </div>
                      <div>
                        <p
                          className={`font-bold ${selectedInterest?.id === area.id ? "text-blue-700" : "text-slate-800"}`}
                        >
                          {area.label}
                        </p>
                      ...
                      {selectedInterest?.id === area.id && (
                        <Zap className="ml-auto text-blue-600 w-4 h-4 fill-blue-600" />
                      )}
                    </button>
                  );
                })}
            ...
            <div className="space-y-6">
                  ...
                  >
                    ...
                    <div className="p-5 bg-white border-2 border-slate-100 rounded-2xl transition-all peer-checked:border-blue-600 peer-checked:bg-blue-50/30 group-hover:border-blue-200">
                      <div className="flex items-center justify-between">
                        ...
                        <div
                          className={`w-6 h-6 rounded-full border-2 transition-all flex items-center justify-center ${selectedLevel?.id === level.id ? "border-blue-600" : "border-slate-200"}`}
                        >
                          <div
                            className={`w-3 h-3 rounded-full bg-blue-600 transition-all ${selectedLevel?.id === level.id ? "scale-100" : "scale-0"}`}
                          />
                        </div>
                      </div>
                    </div>
              ...
              {selectedInterest && (
                <div className="mt-8 p-5 bg-slate-900 rounded-[1.5rem] text-white animate-in slide-in-from-bottom-4 duration-500 relative overflow-hidden">
                  ...
                  <p className="text-[10px] uppercase font-bold text-blue-400 tracking-widest relative z-10">
                    Selected Goal
                  </p>
          ...
          <div className="mt-12 pt-8 border-t border-slate-100 flex justify-between items-center">
            ...
            >
              ...
              <Zap
                className={`w-5 h-5 ${selectedInterest && selectedLevel ? "text-blue-200" : "text-slate-400"}`}
              />
            </button>
          </div>
        </div>
